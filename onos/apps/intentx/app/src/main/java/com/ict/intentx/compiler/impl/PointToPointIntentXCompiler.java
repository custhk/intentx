...
package com.ict.intentx.compiler.impl;


import com.google.common.collect.ImmutableSet;
import com.ict.intentx.utils.DetermineHelper;
...
import org.onosproject.net.intent.ExtendedPointToPointIntent;
...
import static com.ict.intentx.mgr.intf.IntentXService.INTENT_EXTEND_PROVIDER_IDA;
 ...
 * @version 1.0
 * @since 19-3-11 下午9:03
 */
@Component(immediate = true)
public class PointToPointIntentXCompiler
        extends IntentXConnectivityIntentCompiler<ExtendedPointToPointIntent> {

    //private static final ProviderId PID = new ProviderId("apps", "org.onosproject.intentextend", true);
    private static final ProviderId PID = INTENT_EXTEND_PROVIDER_IDA;
    ...
    public void activate() {
        intentManager.registerCompiler(ExtendedPointToPointIntent.class, this);
    }

    @Deactivate
    public void deactivate() {
        intentManager.unregisterCompiler(ExtendedPointToPointIntent.class);
    }

    @Override
    public List<Intent> compile(ExtendedPointToPointIntent intent, List<Intent> installable) {
        ...
        // add my way point path function
        if (DetermineHelper.requireWayPointPath(intent)) {
            return createWayPointCollectionIntent(intent);
        }
    ...
    private void allocateIntentBandwidth(ExtendedPointToPointIntent intent, Path path) {
    ...
    private List<Intent> createZeroHopIntent(ConnectPoint ingressPoint,
                                             ...
                                             ExtendedPointToPointIntent intent) {
    ...
    private List<Intent> createZeroHopLinkCollectionIntent(ExtendedPointToPointIntent intent) {
        return asList(createLinkCollectionIntent(ImmutableSet.of(), DEFAULT_COST,
                intent));
    }

    private List<Intent> createUnprotectedLinkCollectionIntent(ExtendedPointToPointIntent intent) {
     ...
     * with this intent.
     */
    private PortNumber getPrimaryPort(ExtendedPointToPointIntent intent) {
     ...
     */
    private List<FlowRule> createFailoverFlowRules(ExtendedPointToPointIntent intent) {
    ...
    private List<Intent> filterInvalidSubIntents(List<Intent> oldInstallables,
                                                 ExtendedPointToPointIntent pointIntent) {
        List<Intent> intentList = new ArrayList<>();
    ...
    // failover group.
    private void updateFailoverGroup(ExtendedPointToPointIntent pointIntent) {
    ...
    private void updateFailoverGroup(ExtendedPointToPointIntent intent, List<Link> links) {
     ...
     */
    private boolean pathAvailable(ExtendedPointToPointIntent intent) {
    ...
    private List<Intent> createWayPointCollectionIntent(ExtendedPointToPointIntent intent) {
        Path path = getWayPointPath(intent, intent.filteredIngressPoint().connectPoint().elementId(),
                intent.filteredEgressPoint().connectPoint().elementId());
        // Allocate bandwidth if a bandwidth constraint is set
        //allocateIntentBandwidth(intent, path);
