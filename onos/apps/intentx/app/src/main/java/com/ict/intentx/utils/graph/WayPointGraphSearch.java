...
package com.ict.intentx.utils.graph;
 ...
 * @version 1.0
 * @since 19-3-12 下午4:07
 */
public class WayPointGraphSearch<V extends Vertex, E extends Edge<V>>
    ...
    protected SpanningTreeResult internalSearch(Graph<V, E> graph, V src, V dst,
                                                EdgeWeigher<V, E> weigher,  Map<V, Integer> wayPoints, int maxPaths) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (vertex.equals(dst)) {
                // If we have reached our destination, bail and pass all wayPoints
                break;
            }

            //current cost to reach the specified vertex
            ...
            for (E edge : graph.getEdgesFrom(vertex)) {
                ...
                if (!result.hasCost(nextVertex)) {
                    ...
                    Integer tempIndex = wayPoints.get(nextVertex);
                    if (tempIndex != null) {
                        //right order index update
