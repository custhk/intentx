 ...
 * @version 1.0
 * @since 19-3-14 上午11:14
 */
public abstract class IntentXConnectivityIntentCompiler<T extends ConnectivityIntent>
    ...
    @Reference(cardinality = ReferenceCardinality.MANDATORY)
    protected IntentXPathService intentXPathService;
    ...
    protected LinkWeigher weigher(List<Constraint> constraints) {
        return new ConstraintBasedLinkWeigherX(constraints);
    }

    protected LinkWeigherX extendedWeigher(List<Constraint> constraints) {
        return new ConstraintBasedLinkWeigherX(constraints);
    }
    ...
    /**** add mysel function for waypoint route ***/

    /**
     * Computes a waypoint path between two ConnectPoints.
     *
     ...
     */
    protected Path getWayPointPath(ConnectivityIntent intent,
                                   ElementId one, ElementId two) {
        Set<Path> paths = intentXPathService.getWayPointPaths(one, two, extendedWeigher(intent.constraints()));
     ...
     */
    protected class ConstraintBasedLinkWeigherX extends DefaultEdgeWeigher<TopologyVertex, TopologyEdge>
            implements LinkWeigherX {
        @Override
         ...
         * @param constraints path constraints
         */
        ConstraintBasedLinkWeigherX(List<Constraint> constraints) {
            if (constraints == null) {
